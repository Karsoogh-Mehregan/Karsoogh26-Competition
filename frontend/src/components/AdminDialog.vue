...
async function grantExtraTime() {
  ...
  try {
    const wasPaused = settings.value?.status === 'paused'
    const result = await extend(minutes)
    duration.value = String(result.duration_minutes)
    toast.success(
      `${minutes} دقیقه وقت اضافه ثبت شد — مدت کل: ${result.duration_minutes} دقیقه` +
        (wasPaused ? '؛ بازی از سر گرفته شد.' : ''),
    )
  } catch (error) {
    ...
    <DialogContent class="flex max-h-[85dvh] flex-col gap-4 sm:max-w-md" dir="rtl">
      ...
      <div v-else-if="settings" class="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto">
        ...
        <section class="flex flex-col gap-2 rounded-md border p-3">
          ...
          <p class="text-muted-foreground text-xs">
            اگر بازی متوقف باشد، با ثبت وقت اضافه دوباره «در حال اجرا» می‌شود. بازی
            شروع‌نشده یا تمام‌شده با این دکمه اجرا نمی‌شود.
          </p>
        </section>
