...
<template>
  <aside class="flex h-full w-full shrink-0 flex-col overflow-hidden border-e bg-card">
    <header v-if="!isPlayer" class="border-b px-5 py-4">
      <h1 v-if="me" class="text-lg font-bold">تیم‌ها</h1>
      <p v-if="isMentor && !actingTeam" class="text-muted-foreground mt-1 text-sm">
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden px-5 py-4">
      ...
      <div v-else-if="isPlayer" class="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
        <div class="rounded-xl border px-4 py-3">
          <p class="text-muted-foreground text-xs">تیم شما</p>
      ...
      <template v-else-if="isMentor">
        ...
        <p
          v-else-if="listedTeams.length === 0"
          class="text-muted-foreground mt-3 shrink-0 text-sm"
        >
          تیمی پیدا نشد.
        </p>
