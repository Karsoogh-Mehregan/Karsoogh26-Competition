...
class IsOlympicsParticipant(BasePermission):
    """Participating teams and mentors may inspect a physical match."""

    message = "شما به این مسابقه فیزیکی دسترسی ندارید."
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user.has_perm(MENTOR_PERM):
            return True
        return request.method in SAFE_METHODS and user.team_id in (
            obj.player_one_id,
            obj.player_two_id,
        )
